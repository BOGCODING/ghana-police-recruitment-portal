...
const PDFService = {
  ...
  async generateApplicationPDF(data, outputPath = null) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.rect(25, topY, 320, 80).fill(cardBg);
        
        // App ID & Status
        doc.fillColor(accent).font('Helvetica-Bold').fontSize(8.5).text('APPLICATION IDENTIFICATION', 35, topY + 8);
        ...
        if (application?.category !== 'GENERAL_DUTY') {
          ...
          doc.fillColor('#333333').font('Helvetica-Bold').fontSize(8.5).text((application?.category || 'N/A').replace(/_/g, ' '), 35, topY + 44);
        }

        doc.fillColor('#666666').font('Helvetica').fontSize(7.5).text('PRE-SCREENING STATUS', 190, topY + 36);
        ...
        doc.fillColor(isEligible ? '#006B3F' : '#CE1126').font('Helvetica-Bold').fontSize(9.5).text(displayStatus, 190, topY + 44);
        ...
        if (passportPhoto) {
          try {
            ...
            if (photoPath && fs.existsSync(photoPath)) {
              doc.image(photoPath, 375, topY, { width: 80, height: 80 });
              doc.rect(375, topY, 80, 80).lineWidth(1.2).strokeColor(accent).stroke();
            } else {
        ...
        if (qrDataUrl && qrDataUrl.includes(',')) {
          try {
            ...
            doc.image(qrBuffer, 465, topY, { width: 80 });
            doc.rect(465, topY, 80, 80).lineWidth(0.5).strokeColor('#DDDDDD').stroke();
            doc.fontSize(6).fillColor('#999999').text('VERIFIED BY GPS', 465, topY + 83, { width: 80, align: 'center' });
        ...
        const drawSection = (title, fields, columns = 2) => {
          ...
          doc.rect(25, doc.y, 545, 15).fill(accent);
        ...
        try {
          ...
          doc.font('Helvetica').fontSize(8.5).fillColor('black').text(contactInfo.residentialAddress || 'N/A', 35, doc.y + 0.5);
          doc.moveDown(0.4);
        ...
        try {
          ...
          if (bece || wassceArr.length > 0 || tertiaryArr.length > 0) {
            ...
            if (bece) {
              doc.fillColor(accent).font('Helvetica-Bold').fontSize(8).text('BECE Results', 40);
              const bFields = [
            ...
            wassceArr.forEach((w, idx) => {
              doc.fillColor(accent).font('Helvetica-Bold').fontSize(8).text(w.isNovdec ? 'WASSCE (Nov/Dec)' : 'WASSCE/SSCE Certificate Details', 40);
              const wFields = [
              ...
              if (Array.isArray(w.results)) {
                ...
                doc.font('Helvetica').fontSize(7).fillColor('#444444').text(res, 50, doc.y, { width: 500 });
                doc.moveDown(0.2);
              }
            ...
            tertiaryArr.forEach(t => {
              doc.moveDown(0.2);
              doc.fillColor(accent).font('Helvetica-Bold').fontSize(8).text('Tertiary Qualification', 40);
              const tFields = [
        ...
        try {
          if (eligibilityReport && eligibilityReport.checks) {
            doc.moveDown(0.3);
            doc.rect(25, doc.y, 545, 15).fill('#F8F9FA');
            doc.fillColor('#1F2937').font('Helvetica-Bold').fontSize(8.5).text('ELIGIBILITY PRE-SCREENING REPORT', 35, doc.y + 3.5);
            doc.moveDown(0.6);

            const failedChecks = eligibilityReport.checks.filter(c => c.status === 'failed');
            
            if (failedChecks.length > 0) {
              doc.fillColor('#CE1126').font('Helvetica-Bold').fontSize(8).text('Potential Reasons for Disqualification:', 40);
              doc.moveDown(0.2);
              failedChecks.forEach(check => {
                doc.fillColor('#CE1126').font('Helvetica').fontSize(7.5).text(`• ${check.name}: ${check.message}`, 50);
                doc.moveDown(0.1);
              });
            } else {
              doc.fillColor('#006B3F').font('Helvetica-Bold').fontSize(8).text('Applicant meets all baseline eligibility requirements.', 40);
            }
            doc.moveDown(0.3);
          }
        ...
        try {
          doc.moveDown(0.5);
          doc.rect(25, doc.y, 545, 15).fill('#EEEEEE');
          doc.fillColor('#333333').font('Helvetica-Bold').fontSize(8.5).text('DECLARATION & SIGNATORY', 35, doc.y + 3.5);
          doc.moveDown(0.6);
          
          doc.fontSize(7.5).font('Helvetica').fillColor('#444444').text(
          ...
          doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#333333').text('Applicant Signature', 35, fY + 4, { width: 175, align: 'center' });
          
          doc.strokeColor('#AAAAAA').lineWidth(0.5).moveTo(375, fY).lineTo(540, fY).stroke();
          doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#333333').text('Date Signed', 375, fY + 4, { width: 165, align: 'center' });
          
          // Absolute bottom footer
          doc.fontSize(6.5).fillColor('#BBBBBB').text('This document is a computer-generated summary of the electronic application submitted via the Ghana Police Service Portal.', 25, 815, { align: 'center', width: 545 });
  ...
  _drawTwoColumnGrid(doc, fields) {
    ...
    fields.forEach((f, i) => {
      ...
      doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#777777').text(`${f[0]}:`, x, y, { width: labelW });
      doc.font('Helvetica-Bold').fontSize(8).fillColor('#222222').text(String(f[1] || 'N/A'), x + labelW, y, { width: 175 });
  ...
  _drawGrid(doc, fields, startX, labelW) {
    fields.forEach(f => {
      const y = doc.y;
      doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#777777').text(`${f[0]}:`, startX, y, { width: labelW });
      doc.font('Helvetica-Bold').fontSize(8).fillColor('#222222').text(String(f[1] || 'N/A'), startX + labelW, y, { width: 350 });
      doc.y = y + 10;
    });
  }
};
