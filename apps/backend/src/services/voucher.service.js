  ...
  async generateBatch(adminId, count, expiresAt, notes = null) {
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const serialPart = crypto.randomBytes(4).toString('hex').toUpperCase();
        const pinPart = crypto.randomBytes(6).toString('hex').toUpperCase();
        const codePart = crypto.randomBytes(12).toString('hex').toUpperCase();

        vouchers.push({
          code: codePart,
          serialNumber: `GPS-${serialPart}`,
          pinCode: pinPart,
