...
6. **Important Part: Environment Variables.** Render will ask you for 3 values:
   ...
   - `CORS_ORIGIN`: For now, type `*` (this tells the backend to accept requests from anywhere).
   - `NEXT_PUBLIC_WS_URL`: Look at your **gps-backend** URL on Render. If it is `https://gps-backend.onrender.com`, then this value should be `wss://gps-backend.onrender.com`.
...
| :--- | :--- | :--- |
| `CORS_ORIGIN` | Use `*` for your first try. | `*` |
| `NEXT_PUBLIC_WS_URL` | Your backend link but with `wss://` | `wss://my-api.onrender.com` |
| `DATABASE_URL` | Your Supabase connection link. | `postgresql://...` |
| `REDIS_URL` | Your Upstash Redis link. | `rediss://...` |
